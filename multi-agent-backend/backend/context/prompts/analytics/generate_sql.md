...
{% if ontology and ontology.get('concepts') %}
# Domain Ontology & Business Concepts

The following business concepts and relationships define domain semantics for this warehouse:
{% for concept in ontology.get('concepts', []) %}
- **{{ concept.get('label') or concept.get('id', '') }}**: {{ concept.get('description', '') | trim }}{% if concept.get('unit') %} (unit: {{ concept.get('unit') }}){% endif %}
{% endfor %}
